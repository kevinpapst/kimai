...
<?php
echo $this->floater()
    ->setTitle($this->translate('editUser'))
    ->setFormAction('../extensions/ki_adminpanel/processor.php')
    ->setFormId('adminPanel_extension_form_editUser')
    ->addTab('general', $this->translate('general'))
    ->addTab('groupstab', $this->translate('groups'))
    ->floaterBegin();
?>
...
<ul>
    ...
    <li>
        ...
        <input class="formfield" type="password" id="password" name="password" size="9"
               id="password"/> <?php echo $this->kga['lang']['minLength'] ?>
        <?php if ($this->user_details['password'] == ""): ?>

            <br/>
            <img src="../skins/<?php echo $this->escape($this->kga['conf']['skin']) ?>/grfx/caution_mini.png"
                 alt="Caution" valign="middle"/>
    ...
    <li>
        ...
        <input class="formfield" type="password" id="retypePassword" name="retypePassword" id="retypePassword"
               size="9"/>
    </li>
    <li>
        <label for="rate"><?php echo $this->kga['lang']['rate'] ?>:</label>
        <input class="formfield" type="text" id="rate" name="rate"
               value="<?php echo $this->escape(number_format($this->user_details['rate'], 2, $this->kga['conf']['decimalSeparator'], "")) ?>"/>
    </li>
    <li>
...
<?php echo $this->floater()->tabContentBegin('groupstab'); ?>
